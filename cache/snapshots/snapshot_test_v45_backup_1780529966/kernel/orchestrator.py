...
class AntimatterOrchestrator:
    ...
    async def execute(self, input_data: Dict = None) -> Dict:
        ...
        if isinstance(plan_a, Exception) or isinstance(plan_b, Exception):
            raise RuntimeError(f"Falha na geração de planos: A={'ok' if not isinstance(plan_a, Exception) else plan_a}, B={'ok' if not isinstance(plan_b, Exception) else plan_b}")
        ...
        return {"status": status, "run_id": run_id, "artifacts": self.artifacts, "phases": executed_phases, "execution_time_ms": int((time.time() - start_time) * 1000)}
